...
Exposing hosts to the Internet allows anyone to reach services that were designed for intra-pool communication only. On pools using [XOSTOR](../../xostor/), LINSTOR satellites listen by default on TCP port 3366 without SSL/TLS certificate and bind to the XAPI management interface by default. An external party could initiate connections to pool satellites and register a rogue LINSTOR controller, blocking all legitimate LINSTOR commands and disrupting storage operations. If a vulnerability exists in the LINSTOR satellite or related services, a remote attacker could exploit it to compromise the control domain on pool hosts and take control of the entire XCP-ng pool (VMs, storage, and management).
:::
